...
let message = function(event) {
  if (event.data.startsWith("Code")) {
    ...
    if (event.data.slice(5, 6) == "P") {
      let tokenNum = parseInt(event.data.slice(7, 8));
      if (tokenNum == 0) {
        let modHand = pNum - hand + 2;
        if (modHand < 1) {
          modHand += players.length;
        }
        let suit = event.data.slice(8, 9);
        let num = event.data.slice(9, 11);
    ...
    let bCard = parseInt(event.data.slice(5, 6));
    let pos = parseInt(event.data.slice(6, 7));
    let modHand = pNum - hand + 2 + (pNum - hand < -1 ? players.length : 0);
